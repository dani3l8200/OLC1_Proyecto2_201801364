...
CLASSESORINTERFACE: 'public' 'class' 'identifier' '{' BODYC '}'  { $$ = new Nodo('CLASSESORINTERFACE'); $$.addChildrens(new Nodo($1)); $$.addChildrens(new Nodo($2)); $$.addChildrens(new Nodo($3)); $$.addChildrens(new Nodo($4)); $$.addChildrens($5); $$.addChildrens(new Nodo($6)); }
        ...
        | 'public' 'interface' error '{' BODYC '}'        { errorList.push(new Error(idError, 'Syntactic error', this._$.first_line, this._$.first_column, yytext)); console.error('Syntactic error: ' + yytext  + ' in the line ' + this._$.first_line + ' and column ' + this._$.first_column); idError++; }
